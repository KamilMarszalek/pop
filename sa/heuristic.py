...
def greedy_fill(state: BoardState, max_cards: int) -> None:
    while (tile := _find_best_isolated_tile(state)) is not None and len(
        state.selected_tiles
    ) < max_cards:
        state.select_tile(tile)


def _find_best_isolated_tile(state: BoardState) -> Tile | None:
    ...
    for i in range(state.n):
        for j in range(state.m):
            ...
            if not state.can_tile_be_selected(tile):
                continue
            score = _weight_per_neighbors(state, tile)
            if score > best_score:
                best_score = score
                best_tile = tile
    return best_tile
