...
class GeneticAlgorithm:
    ...
    def _generate_starting_population(self) -> Population:
        return [
            Unit(len(self.board), self.num_of_cards)
            for _ in range(self.population_count)
        ]

    def _get_population_evaluation(
        self,
    ...
    def run(self) -> tuple[int, list[int]]:
        while not self._stop():
            ...
            self.crossover()
            self.mutation()
            self.m_evaluation = self._get_population_evaluation(
                self.m_population,
            )
